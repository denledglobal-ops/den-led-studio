...
const navItems = [
  ...
  { label: "LED Ekranlar", icon: MonitorPlay, count: "4", target: "screens" },
  { label: "Playlistler", icon: Film, href: "/playlistler" },
  { label: "Yayın Programı", icon: Clock3, href: "/yayin-programi" },
...
export default function Home() {
  ...
  const [commandHistory, setCommandHistory] = useState<Array<{ id: string; screen_id: string; command: string; status: string; created_at: string }>>([]);
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      const [projectResult, screenResult, commandResult] = await Promise.all([
